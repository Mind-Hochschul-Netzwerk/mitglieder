...
class TemplateVariable implements \Iterator, \ArrayAccess, \Countable {
    ...
    public static function create(string $name, mixed $value, string $escapeType = 'html'): mixed
    {
    ...
    public function __call(string $name, array $arguments): mixed
    {
        ...
        if (($this->value instanceof \DateTimeImmutable || $this->value instanceof \DateTime) && $name === 'format') {
            if (isset($arguments[1]) || !empty(static::$timezone)) {
                $timezone = (!empty($arguments[1])) ? new \DateTimeZone($arguments[1]) : static::$timezone;
                $value = clone $this->value;
                $value = $value->setTimeZone($timezone);
            } else {
                $value = $this->value;
            }
            return static::create($name, $value->format($arguments[0]), $this->escapeType);
        }
    ...
    {
        return Engine::htmlEscape($this->raw());
    }
    ...
    public function offsetGet(mixed $offset): mixed
    {
        ...
        return static::create($this->name . '[' . ((string)$offset) . ']', $value, $this->escapeType);
    }
    ...
    public function current(): mixed
    {
        ...
        return static::create($this->name . '[' . $key . ']', $value, $this->escapeType);
    }
    ...
    public function htmlTable(): string
    {
        ...
        $html = "<tr>" . array_reduce(array_keys($this->value[0]), fn($res, $key) => $res . "<th>" . Engine::htmlEscape($key) . "</th>") . "</tr>\n";
        $html .= array_reduce($this->value,
            fn($res, $row) => $res . "<tr>" . array_reduce($row,
                fn($res2, $value) => $res2 . "<td>" . Engine::htmlEscape($value) . "</td>"
            ) . "</tr>\n");
        return "<table>\n$html\n</table>\n";
    }
    ...
    public function htmlList(string $listType = 'ol'): string
    {
        ...
        $html = array_reduce($this->value, fn($res, $value) => $res . $openTag . Engine::htmlEscape($value) . $closeTag . PHP_EOL);
        return match($listType) {
            'ol', 'ul' => "<$listType>\n$html</$listType>\n",
            'table' => "<table>\n<tr><th>" . Engine::htmlEscape($this->name) . "</th></tr>\n$html</table>\n",
            '' => $html,
